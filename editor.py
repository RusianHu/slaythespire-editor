...
def editor_text(key, default=None):
    return EDITOR_TEXT.get(key, default if default is not None else key)
...
def display_relic_tier_name(tier):
    return RELIC_TIER_DISPLAY_NAMES.get(tier, tier)


def load_json_from_jar(zf, path):
    try:
...
def load_game_localization(zf):
    global game_localization
    game_localization = {
        "cards": {},
        "relics": {},
        "potions": {},
    ...
    for section in game_localization:
        merged = {}
        for lang in LOCALIZATION_PRIORITY:
            path = f"localization/{lang}/{section}.json"
            data = load_json_from_jar(zf, path)
            if isinstance(data, dict):
                merged.update(data)
        game_localization[section] = merged


def localized_name(section, entity_id, default=None):
    entry = game_localization.get(section, {}).get(entity_id)
...
class Relic:
    def __init__(self, id, tier, name=None):
        self.id = id
...
def find_desktop_jar():
    ...
    seen = set()
    for candidate in (
        script_dir / "desktop-1.0.jar",
        script_dir.parent / "desktop-1.0.jar",
        cwd / "desktop-1.0.jar",
        cwd.parent / "desktop-1.0.jar",
    ):
    ...
    for candidate in candidates:
        if candidate.is_file():
            print(f"Using Slay the Spire jar: {candidate}")
            return candidate

    searched = "\n".join(str(candidate) for candidate in candidates)
    raise FileNotFoundError(
        "Could not find desktop-1.0.jar.\n"
        "Please place it next to editor.py or in the parent directory.\n"
        f"Searched:\n{searched}"
    )
                    ...
                    if aslist[:4] == ["com", "megacrit", "cardcrawl", "rooms"]:
                        ...
                        if len(aslist) == 6:
                            ...
                            if card_name:
                                ...
                                card_id_re = re.compile(r".*\.field public static final ID Ljava/lang/String; = '([^']+)'")
                                fallback_id_re = re.compile(r".*ldc '([-A-Za-z0-9_ ]*).*'")
                                ...
                                for line in out:
                                    ...
                                    if card_id is None:
                                        card_id_match = card_id_re.match(line)
                                        if card_id_match:
                                            card_id = card_id_match[1]
                                            found = True

                                    if not found and fallback_id is None:
                                        fallback_id_match = fallback_id_re.match(line)
                                        if fallback_id_match:
                                            fallback_id = fallback_id_match[1]
                                            found = True
                                ...
                                card_id = card_id or fallback_id or card_name
                                card_display_name = localized_name("cards", card_id, default=card_id)
                                all_cards[card_id] = Card(card_id, card_display_name, color, ctype, crarity, ctarget)
                                print(f"Found {all_cards[card_id]}")
                        ...
                        id_re = re.compile(r".*\.field public static final ID Ljava/lang/String; = '([^']+)'")
                        fallback_id_re = re.compile(r".*ldc '([^']+)'.*")
                        ...
                        with zf.open(pathfn) as card_file:
                            ...
                            for line in out:
                                ...
                                for regexp, var in (
                                        (tier_re, 'tier'),
                                        (id_re, 'id'),
                                        (fallback_id_re, 'fallback_id')):

                                    if var not in myvars:
                                        rematch = regexp.match(line)
                                        if rematch:
                                            # print(f"{regexp} ?= {line}")
                                            myvars[var] = rematch[2] if var == 'tier' else rematch[1]
                                            found = True

                            relic_id = myvars.get("id") or myvars.get("fallback_id") or aslist[4].split('.')[0]
                            if "tier" in myvars:
                                relic_display_name = localized_name("relics", relic_id, default=relic_id)
                                r = Relic(relic_id, myvars["tier"], relic_display_name)
                                all_relics[r.id] = r
                            else:
                        ...
                        if not aslist[4] or "$" in aslist[4] or aslist[4] in ['AbstractPotion.class']:
                            continue

                        id_re = re.compile(r".*\.field public static final POTION_ID Ljava/lang/String; = '([^']+)'")
                        fallback_id_re = re.compile(r".*ldc '([A-Za-z ]*)'.*")
                        ...
                        with zf.open(pathfn) as potion_file:
                            ...
                            potion_id = myvars.get("id") or myvars.get("fallback_id")
                            if potion_id:
                                potion_display_name = localized_name("potions", potion_id, default=potion_id)
                                p = Potion(potion_id, potion_display_name)
                                all_potions[p.id] = p
                            else:
                                print(f'No matches for {id_re} or {fallback_id_re}')
        ...
        for setting_key in saveobj:
            ...
            for widget_dict in [settings_dict, ]:
                if setting_key in widget_dict:
                    ...
                    value = []
                    for relic in relics:
                        value.append(relic.id)

                elif setting_key == "potions":
                    value = []
                    for potion in potions:
                        value.append(potion.id)
...
class ColorPanel(wx.ScrolledWindow):
    ...
    def add_cards(self, color):
        for card in sorted(all_cards.values()):
            if card.color == color:
...
class LibraryPanel(wx.Notebook):
    def __init__(self, parent, id, *args, **kwargs):
        ...
        for color in colors:
            ...
            self.AddPage(self.color[color], display_color_name(color))
...
class AllRelicPanel(wx.Notebook):
    ...
    def __init__(self, parent, id, *args, **kwargs):
        ...
        for tier in RELIC_TIERS:
            ...
            self.AddPage(getattr(self, tier), display_relic_tier_name(tier))
...
class AllPotionPanel(wx.ScrolledWindow):
    ...
    def add_potions(self):
        for potion_obj in sorted(all_potions.values(), key=lambda item: item.name):
...
class MainFrame(wx.Frame):
    ...
    def on_open(self, event):
        ...
        with wx.FileDialog(self, editor_text("open_autosave", "Open autosave file"), wildcard="autosave files (*.autosave*)|*.autosave*",
    ...
    def __init__(self, parent):
        ...
        wx.Frame.__init__(
            ...
            parent=parent,
            id=wx.ID_ANY,
            title=editor_text("title", "Save the Spire Save Editor"),
        ...
        self.TabPanel.AddPage(self.Settings, editor_text("tab_settings", "Settings"))
        ...
        self.TabPanel.AddPage(self.Cards, editor_text("tab_cards", "Cards"))
        ...
        self.TabPanel.AddPage(self.Relics, editor_text("tab_relics", "Relics"))
        ...
        self.TabPanel.AddPage(self.Potions, editor_text("tab_potions", "Potions"))
        ...
        self.TabPanel.AddPage(self.Metrics, editor_text("tab_metrics", "Metrics"))
    ...
    def menu_bar(self):
        ...
        menu = wx.Menu()
        file_open = menu.Append(wx.ID_ANY, editor_text("menu_open", "&Open"), "", wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.on_open, id=file_open.GetId())

        file_save = menu.Append(wx.ID_ANY, editor_text("menu_save", "&Save"), "", wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.on_save, id=file_save.GetId())

        menu.Append(wx.ID_ANY, editor_text("menu_close_file", "&Close file"), "")
        menu.AppendSeparator()

        exit = menu.Append(wx.ID_ANY, editor_text("menu_exit", "E&xit"), "", wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.onExit, id=exit.GetId())

        self.frame_menubar.Append(menu, editor_text("menu_file", "&File"))
